...
# ============================================================================
# BENCHMARK CITY SIZE (adjust to stress-test how many buildings the viewer
# can handle). The curated layout ships 100 hand-placed buildings; set this
# lower to render only a subset of them, or higher to auto-generate extra
# buildings on a procedural grid beyond the curated 100 so you can push the
# count up and find the rendering/performance limit.
# ============================================================================
NUM_BUILDINGS = 100
...
def _video_fingerprint(video_path: str) -> str:
    """Short hash of the source file's size + mtime, so any replacement of
    background.mp4 (even with a same-size or smaller file) is detected as a
    different video instead of silently reusing a stale re-encoded copy."""
    stat = os.stat(video_path)
    raw = f"{video_path}:{stat.st_size}:{stat.st_mtime_ns}"
    return hashlib.sha1(raw.encode("utf-8")).hexdigest()[:12]


@st.cache_data(show_spinner=False)
def get_web_video_base64(video_path: str, fingerprint: str) -> str:
    ...
    # Fingerprint is baked into the filename, so a new/edited source video
    # always gets its own encoded file instead of matching an old one.
    web_video = os.path.join(
        cache_dir,
        f"background_web_{fingerprint}.mp4"
    )

    # Sweep out encoded copies of previous videos - only the current
    # fingerprint's file should stick around.
    for f in os.listdir(cache_dir):
        if f.startswith("background_web_") and f != os.path.basename(web_video):
            try:
                os.remove(os.path.join(cache_dir, f))
            except OSError:
                pass

    # Re-encode only if this exact fingerprint hasn't been encoded before.
    if not os.path.exists(web_video):
        try:
            subprocess.run(
                [
                    "ffmpeg",
...
def set_local_bg_video(video_path: str, overlay_opacity: float = 0.55):
    ...
    b64_video = get_web_video_base64(video_path, _video_fingerprint(video_path))
...
def _auto_extra_building_configs(count: int, start_index: int) -> list:
    """Procedurally generate extra building configs beyond the 100 curated
    ones, scattered randomly (not a rigid grid) so a large NUM_BUILDINGS
    doesn't just look like a repeating tile pattern. Placed east of the
    curated city; the ground slab is sized to fit everything afterwards so
    these buildings never end up floating with no ground beneath them.
    """
    rng = pyrandom.Random()  # unseeded -> a fresh random layout each regeneration
    pal_keys = list(INDIAN_PALETTES.keys())
    pal_weights = [1 if k == "under_construction" else 4 for k in pal_keys]
    roof_styles = ["water_tank", "solar_farm", "hvac_cube"]

    # Scatter field grows with the building count so density stays roughly
    # constant instead of everything piling into one small square.
    field_x0, field_y0 = 250.0, 0.0
    field_side = max(200.0, (count ** 0.5) * 22.0)

    extra = []
    for i in range(count):
        idx = start_index + i
        w = round(rng.uniform(7.0, 18.0), 1)
        d = round(rng.uniform(7.0, 18.0), 1)
        bx = round(field_x0 + rng.uniform(0.0, field_side), 1)
        by = round(field_y0 + rng.uniform(0.0, field_side), 1)

        pal_key = rng.choices(pal_keys, weights=pal_weights, k=1)[0]
        floors_cnt = rng.randint(1, 9)
        roof = rng.choice(roof_styles)
        awning = rng.random() < 0.5
        tank_col = rng.choice(["#0F172A", "#F8FAFC"])
        balc = rng.random() < 0.65
        under_c = (pal_key == "under_construction")
        b_id = f"BLD-{idx + 1:04d}"

        extra.append((b_id, pal_key, bx, by, w, d, floors_cnt, roof, awning, tank_col, balc, under_c))

    return extra


# ============================================================================
# 3. EXTENDED DENSE INDIAN BENCHMARK CITY (100 REALISTIC BUILDINGS)
# ============================================================================

@st.cache_data(show_spinner=False)
def generate_synthetic_city(num_buildings: int = NUM_BUILDINGS) -> CityModel:
    buildings: Dict[str, Building] = {}
    names = [
    ...
    configs = [
        ...
        # Sector 11: Southern Extension - West Corridor (mirrors Sector 1,
        # 144m further south, across the new arterial at y=140)
        ("BLD-0065", "weathered_plaster", 6, 150, 14, 16, 3, "water_tank", True, "#0F172A", True, False),
        ("BLD-0066", "exposed_brick", 22, 150, 12, 14, 2, "solar_farm", True, "#F8FAFC", True, False),
        ("BLD-0067", "commercial_glass", 6, 168, 16, 12, 4, "hvac_cube", False, "#0F172A", False, False),
        ("BLD-0068", "painted_terracotta", 24, 168, 11, 13, 3, "water_tank", True, "#0F172A", True, False),
        ("BLD-0069", "under_construction", 7, 184, 8, 8, 2, "water_tank", False, "#0F172A", False, True),
        ("BLD-0070", "sandstone_yellow", 16, 184, 7, 8, 1, "solar_farm", True, "#F8FAFC", False, False),
        ("BLD-0071", "exposed_brick", 24, 184, 8, 8, 2, "water_tank", True, "#0F172A", True, False),

        # Sector 12: Southern Extension - Mid-West Urban Blocks
        ("BLD-0072", "monsoon_concrete", 44, 150, 15, 15, 6, "hvac_cube", False, "#0F172A", True, False),
        ("BLD-0073", "painted_terracotta", 61, 150, 13, 13, 4, "water_tank", True, "#0F172A", True, False),
        ("BLD-0074", "weathered_plaster", 44, 167, 12, 15, 3, "water_tank", True, "#F8FAFC", True, False),
        ("BLD-0075", "commercial_glass", 58, 167, 16, 13, 5, "solar_farm", False, "#0F172A", False, False),
        ("BLD-0076", "sandstone_yellow", 44, 184, 7, 8, 1, "water_tank", True, "#0F172A", False, False),
        ("BLD-0077", "monsoon_concrete", 52, 184, 8, 8, 1, "water_tank", True, "#0F172A", False, False),
        ("BLD-0078", "exposed_brick", 61, 184, 7, 8, 2, "water_tank", True, "#F8FAFC", True, False),

        # Sector 13: Southern Extension - Central High-Density Hub
        ("BLD-0079", "commercial_glass", 83, 150, 15, 17, 7, "hvac_cube", False, "#0F172A", False, False),
        ("BLD-0080", "sandstone_yellow", 100, 150, 13, 14, 3, "solar_farm", True, "#F8FAFC", True, False),
        ("BLD-0081", "monsoon_concrete", 83, 169, 12, 13, 5, "water_tank", True, "#0F172A", True, False),
        ("BLD-0082", "weathered_plaster", 97, 168, 16, 14, 4, "water_tank", True, "#0F172A", True, False),
        ("BLD-0083", "under_construction", 83, 184, 8, 8, 2, "water_tank", False, "#0F172A", False, True),
        ("BLD-0084", "painted_terracotta", 93, 184, 9, 8, 1, "solar_farm", True, "#F8FAFC", False, False),
        ("BLD-0085", "exposed_brick", 108, 184, 7, 8, 2, "water_tank", True, "#0F172A", True, False),

        # Sector 14: Southern Extension - Eastern Tech Extension
        ("BLD-0086", "commercial_glass", 123, 150, 16, 18, 9, "hvac_cube", False, "#0F172A", False, False),
        ("BLD-0087", "monsoon_concrete", 141, 150, 14, 15, 6, "solar_farm", True, "#F8FAFC", True, False),
        ("BLD-0088", "weathered_plaster", 123, 170, 13, 14, 4, "water_tank", True, "#0F172A", True, False),
        ("BLD-0089", "exposed_brick", 138, 169, 16, 15, 3, "water_tank", True, "#F8FAFC", True, False),
        ("BLD-0090", "sandstone_yellow", 124, 186, 8, 8, 2, "water_tank", True, "#0F172A", False, False),
        ("BLD-0091", "painted_terracotta", 134, 186, 9, 8, 2, "solar_farm", True, "#0F172A", True, False),
        ("BLD-0092", "under_construction", 145, 186, 9, 8, 3, "water_tank", False, "#0F172A", False, True),

        # Sector 15: Southern Civic Arterial (mirrors Sector 9, closes out
        # the new district across the far southern edge of the city)
        ("BLD-0093", "sandstone_yellow", 6, 206, 14, 15, 4, "water_tank", True, "#0F172A", True, False),
        ("BLD-0094", "exposed_brick", 28, 206, 13, 14, 3, "solar_farm", True, "#F8FAFC", True, False),
        ("BLD-0095", "monsoon_concrete", 52, 206, 16, 16, 6, "hvac_cube", False, "#0F172A", True, False),
        ("BLD-0096", "commercial_glass", 78, 206, 14, 15, 5, "solar_farm", False, "#0F172A", False, False),
        ("BLD-0097", "weathered_plaster", 104, 206, 15, 16, 4, "water_tank", True, "#0F172A", True, False),
        ("BLD-0098", "commercial_glass", 131, 206, 17, 16, 6, "hvac_cube", False, "#0F172A", True, False),

        # Sector 16: Southern Infill Micro-Parcels
        ("BLD-0099", "exposed_brick", 34, 184, 7, 7, 2, "water_tank", True, "#0F172A", False, False),
        ("BLD-0100", "sandstone_yellow", 74, 184, 6, 7, 1, "solar_farm", True, "#F8FAFC", False, False),
    ]

    # Apply the adjustable NUM_BUILDINGS global (passed in as num_buildings,
    # which also keys the @st.cache_data cache so changing it actually takes
    # effect on rerun): trim to a subset of the curated 100 if lower, or
    # bolt on procedurally-generated buildings if higher, so the benchmark
    # city's size can be pushed to find rendering/performance limits.
    if num_buildings <= len(configs):
        configs = configs[:max(0, num_buildings)]
    else:
        configs = configs + _auto_extra_building_configs(num_buildings - len(configs), len(configs))
    ...
    tree_coords = [
        ...
        # Southern extension canopy (Sectors 11-16)
        (35.0, 145.0, 4.8, "#15803D"), (74.0, 145.5, 1.5, "#166534"),
        (2.0, 196.0, 4.2, "#15803D"), (115.0, 196.0, 4.6, "#166534"),
        (40.0, 225.0, 5.0, "#15803D"), (115.0, 225.0, 4.4, "#166534"),
        (155.0, 160.0, 5.2, "#15803D"), (75.0, 235.0, 4.9, "#166534")
    ]
    ...
    # Size the ground slab to whatever buildings actually exist instead of a
    # fixed footprint, so trimming NUM_BUILDINGS down or scattering random
    # extra buildings out past the curated sectors never leaves buildings
    # floating with no ground underneath them.
    if buildings:
        gx_min = min(b.cx - b.width / 2.0 for b in buildings.values())
        gx_max = max(b.cx + b.width / 2.0 for b in buildings.values())
        gy_min = min(b.cy - b.depth / 2.0 for b in buildings.values())
        gy_max = max(b.cy + b.depth / 2.0 for b in buildings.values())
    else:
        gx_min, gx_max, gy_min, gy_max = -13.0, 177.0, -3.5, 231.5

    ground_margin = 20.0
    ground_w = (gx_max - gx_min) + ground_margin * 2
    ground_d = (gy_max - gy_min) + ground_margin * 2
    ground_cx = (gx_min + gx_max) / 2.0
    ground_cy = (gy_min + gy_max) / 2.0

    g_lon, g_lat = local_to_wgs84(ground_cx, ground_cy, DEFAULT_REF_LON, DEFAULT_REF_LAT)
    synth_ground = [{
        "lon": g_lon, "lat": g_lat, "w": ground_w, "d": ground_d, "z": -0.15, "h": 0.15, "color": "#1E293B"
    }]
    ...
    return CityModel(
        ...
        road_y_coords=[38.0, 57.0, 107.0, 140.0, 200.0],
...
def compile_infrastructure_elements(city: CityModel) -> Tuple[List[dict], List[dict]]:
    ...
    for xr in city.road_x_coords:
        r_lon, r_lat = local_to_wgs84(xr, y_center, ref_lon, ref_lat)
        infra_list.append({
            "lon": r_lon, "lat": r_lat, "w": road_w, "d": y_span, "z": 0.0, "h": 0.08,
            "color": "#1E293B"
        })
        for sw_x in [xr - road_w / 2 - sw_w / 2, xr + road_w / 2 + sw_w / 2]:
            s_lon, s_lat = local_to_wgs84(sw_x, y_center, ref_lon, ref_lat)
            infra_list.append({
    ...
    rickshaw_coords = [
        ...
        # Southern extension corridors
        (35.5, 158.0), (114.5, 162.0), (74.0, 216.0), (140.0, 192.0)
    ]
    ...
    cart_coords = [
        (35.5, 42.0), (74.2, 50.0), (114.2, 48.0), (74.5, 110.0),
        (35.0, 192.0), (114.5, 216.0)
    ]
...
@st.cache_data(show_spinner=False)
def _texture_to_data_uri(abs_path: str, mtime_ns: int) -> Optional[str]:
    """Base64-embed a local texture file as a data: URI.

    Cesium's ImageMaterialProperty runs in the browser and can't reach the
    Streamlit process's local filesystem, so a plain relative path (e.g.
    'assets/facades/residential/x.jpg') 404s and silently falls back to a
    flat white material. Embedding the bytes directly sidesteps that,
    the same way the background video and QR code are already handled.
    `mtime_ns` is only there to bust the cache if the file is replaced.
    """
    ext = os.path.splitext(abs_path)[1].lower()
    mime = _TEXTURE_MIME_MAP.get(ext)
    if not mime:
        return None
    try:
...
def render_cesium_viewer(
    ...
    def resolve_facade_slice(typology_key: str, floor_index: int = 1, is_awning_active: bool = False) -> Optional[str]:
        ...
        if os.path.isdir(target_folder):
            ...
            if matched:
                ...
                return _texture_to_data_uri(chosen_abs, os.stat(chosen_abs).st_mtime_ns)
    ...
    if boxes:
        ...
        target_range = 150.0 if level == "CITY" else (70.0 if level == "BUILDING" else 35.0)
    else:
        target_lon, target_lat, target_range = ref_lon, ref_lat, 300.0
...
def make_qr_code_png_b64(payload: str) -> Optional[str]:
    if not HAS_QRCODE:
        return None
    # border=4 is the QR spec's minimum "quiet zone" — anything less and a
    # lot of phone scanner apps fail to even detect the code is there.
    # Plain black/white (rather than the app's cyan/navy theme colors) is
    # used here too, since some scanners threshold on true black/white and
    # can misread tinted codes.
...
# ============================================================================
# 4b. INTERIOR ROOM SUBDIVISION (on-demand, selected unit only)
# ============================================================================
# Deliberately NOT baked into every PropertyUnit at generation time - a full
# room breakdown for every unit in every floor of every building would mean
# generating and rendering geometry for thousands of rooms that are never
# looked at. Instead this runs only for whichever single unit is currently
# open in the UNIT-level viewer, keyed off the cadastral property_type that
# generate_architectural_floorplan() already assigns.

_INTERIOR_ROOM_TEMPLATES: Dict[str, List[Tuple[str, float, float, float, float]]] = {
    # Each room is (name, fx0, fy0, fx1, fy1) as a fraction of the unit's
    # own width (x, 0=left..1=right) and depth (y, 0=back..1=front).
    "Residential 1BHK": [
        ("Bedroom", 0.00, 0.00, 0.55, 1.00),
        ("Living / Hall", 0.55, 0.40, 1.00, 1.00),
        ("Kitchen", 0.55, 0.00, 0.80, 0.40),
        ("Bathroom", 0.80, 0.00, 1.00, 0.40),
    ],
    "Residential 2BHK": [
        ("Bedroom 1", 0.00, 0.45, 0.60, 1.00),
        ("Bedroom 2", 0.00, 0.00, 0.60, 0.45),
        ("Living / Hall", 0.60, 0.45, 1.00, 1.00),
        ("Kitchen", 0.60, 0.00, 0.82, 0.45),
        ("Bathroom", 0.82, 0.00, 1.00, 0.45),
    ],
    "Residential 3BHK": [
        ("Bedroom 1", 0.00, 0.65, 0.62, 1.00),
        ("Bedroom 2", 0.00, 0.32, 0.62, 0.65),
        ("Bathroom 1", 0.00, 0.00, 0.30, 0.32),
        ("Bedroom 3", 0.30, 0.00, 0.62, 0.32),
        ("Living / Hall", 0.62, 0.40, 1.00, 1.00),
        ("Kitchen", 0.62, 0.00, 0.82, 0.40),
        ("Bathroom 2", 0.82, 0.00, 1.00, 0.40),
    ],
}


def get_interior_room_template(property_type: str) -> Optional[List[Tuple[str, float, float, float, float]]]:
    """Fractional room layout for a unit's cadastral type, or None for
    types that stay open-plan (shops get one commercial + one back-of-house
    split; common corridors/stairwell/storage aren't subdivided - a real
    lobby or storage room isn't split into "rooms")."""
    if property_type in _INTERIOR_ROOM_TEMPLATES:
        return _INTERIOR_ROOM_TEMPLATES[property_type]
    if property_type.startswith("Ground Commercial Shop"):
        return [
            ("Shop Floor", 0.00, 0.22, 1.00, 1.00),
            ("Counter / Storage", 0.00, 0.00, 1.00, 0.22),
        ]
    return None


def build_unit_interior_overlays(bld: "Building", flr: "Floor", unit: "PropertyUnit",
                                  city: CityModel) -> Tuple[List[dict], List[dict]]:
    """
    Procedurally subdivides ONE unit into rooms based on its property_type,
    returning (interior_wall_overlay_boxes, room_schedule). Shared edges
    between adjacent rooms are only drawn once, and edges that coincide with
    the unit's own outer boundary are skipped (that wall already exists
    implicitly as the unit's own box).
    """
    template = get_interior_room_template(unit.property_type)
    if not template:
        return [], [{"name": unit.property_type, "area_sqm": unit.area_sqm}]

    wall_h = max(1.8, min(2.6, (unit.z_max - unit.z_min) - 0.25))
    wall_t = 0.10
    base_z = unit.z_min + 0.02

    seen_edges = set()
    walls: List[dict] = []
    rooms: List[dict] = []

    def add_edge(x0: float, y0: float, x1: float, y1: float):
        key = (round(x0, 3), round(y0, 3), round(x1, 3), round(y1, 3))
        rkey = (round(x1, 3), round(y1, 3), round(x0, 3), round(y0, 3))
        if key in seen_edges or rkey in seen_edges:
            return
        seen_edges.add(key)

        on_boundary = (
            (abs(x0) < 1e-6 and abs(x1) < 1e-6) or (abs(x0 - 1) < 1e-6 and abs(x1 - 1) < 1e-6) or
            (abs(y0) < 1e-6 and abs(y1) < 1e-6) or (abs(y0 - 1) < 1e-6 and abs(y1 - 1) < 1e-6)
        )
        if on_boundary:
            return

        lx0 = unit.cx - unit.w / 2.0 + x0 * unit.w
        lx1 = unit.cx - unit.w / 2.0 + x1 * unit.w
        ly0 = unit.cy - unit.d / 2.0 + y0 * unit.d
        ly1 = unit.cy - unit.d / 2.0 + y1 * unit.d

        is_vertical = abs(x0 - x1) < 1e-6
        seg_len = max(0.15, abs(ly1 - ly0) if is_vertical else abs(lx1 - lx0))
        mid_x, mid_y = (lx0 + lx1) / 2.0, (ly0 + ly1) / 2.0
        lon, lat = local_to_wgs84(mid_x, mid_y, city.ref_lon, city.ref_lat)

        walls.append({
            "id": f"{unit.unit_id}-IWALL-{len(walls)}",
            "name": f"{unit.unit_id} — interior wall",
            "lon": lon, "lat": lat,
            "w": wall_t if is_vertical else seg_len,
            "d": seg_len if is_vertical else wall_t,
            "base_z": base_z, "h": wall_h,
            "color": "#E2E8F0", "alpha": 0.96, "outline_color": "#94A3B8",
            "metadata": {"Type": "Interior Partition Wall", "Unit": unit.unit_id, "Floor": flr.floor_id}
        })

    for (name, fx0, fy0, fx1, fy1) in template:
        add_edge(fx0, fy0, fx1, fy0)
        add_edge(fx1, fy0, fx1, fy1)
        add_edge(fx1, fy1, fx0, fy1)
        add_edge(fx0, fy1, fx0, fy0)
        rooms.append({"name": name, "area_sqm": round((fx1 - fx0) * (fy1 - fy0) * unit.area_sqm, 1)})
...
if "city_model" not in st.session_state:
    st.session_state.city_model = generate_synthetic_city(NUM_BUILDINGS)
...
# Programmatic tab-switch requests (e.g. "OPEN IN 3D VIEWER" -> jump to
# Digital Passport) go through this flag instead of writing
# st.session_state.workspace_mode directly, because the selectbox below is
# bound to that key with key="workspace_mode" - Streamlit forbids
# overwriting a widget-bound session_state key after that widget has been
# instantiated in the same run. Setting it here, before the widget exists
# for this run, is safe; the requester just sets the flag and calls
# st.rerun().
if st.session_state.get("_pending_workspace_mode"):
    st.session_state.workspace_mode = st.session_state.pop("_pending_workspace_mode")
...
with st.sidebar:
    ...
    st.markdown("## MATRIX")
    st.caption("A One-Stop Platform for")
...
# On-demand interior room subdivision - only computed for whichever single
# unit is currently open, never for the whole city at once (see
# build_unit_interior_overlays for why).
_active_unit_rooms: List[dict] = []
if _current_level == "UNIT" and u_id and b_id in _city.buildings:
    _bld_for_unit = _city.buildings[b_id]
    _flr_for_unit = next((f for f in _bld_for_unit.floors if f.floor_id == f_id), None)
    _unit_for_interior = None
    if _flr_for_unit:
        _unit_for_interior = next((u for u in _flr_for_unit.units if u.unit_id == u_id), None)
    if _flr_for_unit and _unit_for_interior:
        _interior_walls, _active_unit_rooms = build_unit_interior_overlays(
            _bld_for_unit, _flr_for_unit, _unit_for_interior, _city
        )
        _overlay_boxes += _interior_walls
...
crumb = f"MATRIX: A One-Stop Platform for 3D Property Mapping and 3D ULPIN Generation"
if b_id:
    crumb += f" ❯ 🏢 **{b_id}**"
if f_id:
    ...
    if not u_id:
        ...
        if u_p:
            ...
            with pc2:
                ...
                if qr_b64:
                    st.image(f"data:image/png;base64,{qr_b64}", caption="Scan for property record", width=260)
                else:
...
if st.session_state.current_level == "CITY":
    with view_col:
        st.subheader("Interactive Geospacial 3D City Model  (CesiumJS 3D)")
        st.caption("Explore the 3D City or Click any building in 3D to drill down.")
    ...
    with info_col:
        ...
        if st.session_state.selected_unit_id:
            ...
            if active_u:
                ...
                st.markdown("</div>", unsafe_allow_html=True)
                if len(_active_unit_rooms) > 1:
                    st.markdown("**Room Schedule**")
                    for _room in _active_unit_rooms:
                        st.write(f"- {_room['name']} — {_room['area_sqm']} m²")
                if st.button("📇 OPEN DIGITAL PASSPORT", use_container_width=True, key="open_passport_btn"):
                    st.session_state["_pending_workspace_mode"] = "Digital Passport"
                    st.rerun()